...
class DatabaseManager:
    ...
    def __init__(
        ...
        Args:
            ...
            remote_db_url: 远程 MySQL 数据库连接 URL
        """
        ...
        if self.local_db_path:
            try:
                ...
                logger.info(f"本地数据库连接成功: {self.local_db_path}")
    ...
    def search_local_videos_by_keywords(
        ...
        try:
            with self.local_session() as session:
                ...
                logger.info(f"本地数据库搜索到 {len(video_ids)} 个视频")
                return video_ids
        except Exception as e:
            logger.error(f"本地数据库搜索失败: {e}", exc_info=True)
            return []
    ...
    def search_remote_videos_by_keywords(
        ...
        try:
            with self.remote_session() as session:
                ...
                result = session.execute(text(query_str), params)
                video_ids = [row[0] for row in result]
                logger.info(f"远程数据库搜索到 {len(video_ids)} 个视频")
                return video_ids
        except Exception as e:
            logger.error(f"远程数据库搜索失败: {e}", exc_info=True)
            return []
